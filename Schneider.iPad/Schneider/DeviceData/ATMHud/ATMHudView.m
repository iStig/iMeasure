...
- (id)initWithFrame:(CGRect)frame andController:(ATMHud *)c {
    if ((self = [super initWithFrame:frame])) {
		...
		backgroundLayer.backgroundColor = [UIColor colorWithRed:79.0f/255.0f green:166.0f/255.0f blue:0 alpha:p.alpha].CGColor;
...
- (void)calculate {
	if (!caption || [caption isEqualToString:@""]) {
		...
		CGSize captionSize = [caption sizeWithFont:ATM_HHD_FONT constrainedToSize:CGSizeMake(160, 200) lineBreakMode:UILineBreakModeWordWrap];
		...
		if (fixedSize.width > 0 & fixedSize.height > 0) {
			...
			hasFixedSize = YES;
			captionSize = [caption sizeWithFont:ATM_HHD_FONT constrainedToSize:CGSizeMake(s.width-p.margin*2, 200) lineBreakMode:UILineBreakModeWordWrap];
		...
		if (!hasFixedSize) {
			if (p.accessoryPosition == ATMHudAccessoryPositionTop || p.accessoryPosition == ATMHudAccessoryPositionBottom) {
				if (progress > 0) {
					...
					if (captionSize.width+p.margin*2 < progressRect.size.width) {
						captionSize = [caption sizeWithFont:ATM_HHD_FONT constrainedToSize:CGSizeMake(progressRect.size.width, 200) lineBreakMode:UILineBreakModeWordWrap];
						captionRect.size = captionSize;
			...
			if (p.accessoryPosition == ATMHudAccessoryPositionTop || p.accessoryPosition == ATMHudAccessoryPositionBottom) {
				...
				if (marginX < p.margin) {
					captionSize = [caption sizeWithFont:ATM_HHD_FONT constrainedToSize:CGSizeMake(160, 200) lineBreakMode:UILineBreakModeWordWrap];
					captionRect.size = captionSize;
...
- (CGSize)calculateSizeForQueueItem:(ATMHudQueueItem *)item {
	...
	if (!item.caption || [item.caption isEqualToString:@""]) {
		...
		BOOL hasFixedSize = NO;
		CGSize captionSize = [item.caption sizeWithFont:ATM_HHD_FONT constrainedToSize:CGSizeMake(160, 200) lineBreakMode:UILineBreakModeWordWrap];
		...
		if (!hasFixedSize) {
			...
			if (item.accessoryPosition == ATMHudAccessoryPositionTop || item.accessoryPosition == ATMHudAccessoryPositionBottom) {
				...
				marginX = 0.5*deltaWidth;
				if (marginX < p.margin) {
					captionSize = [item.caption sizeWithFont:ATM_HHD_FONT constrainedToSize:CGSizeMake(160, 200) lineBreakMode:UILineBreakModeWordWrap];
